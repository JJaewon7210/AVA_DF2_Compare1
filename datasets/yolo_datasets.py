...
from utils.general import check_requirements, xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, segments2boxes, resample_segments, clean_str
...
def img2label_paths(img_paths):
    ...
    sa, sb = os.sep + 'images', os.sep + 'labels'  # /images, /labels substrings
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def print_summary(self):
        logger.info("Number of images: {}".format(len(self.img_files)))
        logger.info("Image paths: {}".format(self.path))
        logger.info("Label paths: {}".format(self.path.replace('images','labels')))
        logger.info("Image size: {}".format(self.img_size))
        logger.info("Data augmentation: {}".format(self.augment))
...
class DeepFasion2WithPseudoLabel(LoadImagesAndLabels):
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix=''):
        
        logger.info(f"\n====> Loading {type(self).__name__} Dataset")
    ...
    def __getitem__(self, index):
        ...
        # _shape: [B, ]
        img_index = path[-10:-4]
        feature = np.load(self.path_feature+'/'  +img_index+'.npy')
        feature = torch.tensor(feature)
        
        return img, label, path, _shape, feature