...
from datasets.ava_dataset_utils import retry_load_images, get_frame_idx, get_sequence
...
class AvaWithPseudoLabel(Ava):
    def __init__(self, cfg, split, only_detection=False):
        logger.info(f"\n====> Loading {type(self).__name__} Dataset")
        super().__init__(cfg, split, only_detection)

    def __getitem__(self, idx):
        """
        Generate corresponding clips, boxes, labels ,features and metadata for given idx.
        Note that features are pseudo-labels from YOLOv7 which is trained by Deepfashion2.
        Feature has the shape of [ C, W, H, 4(bbox)+1(conf)+13(Class probability) ]

        Args:
            idx (int): the video index provided by the pytorch sampler.
        Returns:
            clip (tensor): the frames of sampled from the video. The dimension
                is `channel` x `num frames` x `height` x `width`.
