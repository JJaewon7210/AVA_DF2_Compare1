...
def bbox_iou_df2(box1, box2, x1y1x2y2=True, mother_box='box2', eps=1e-7):
    # Returns the IoU of box1 to box2. box1 is 4, box2 is nx4
    assert mother_box == 'box1' or mother_box == 'box2', "Invalid mother_box value. Allowed values are 'box1' or 'box2'."
    box2 = box2.T
...
def bbox_iou_numpy(box1, box2, x1y1x2y2_box1=False, x1y1x2y2_box2=False, eps=1e-9):
    ...
    # Get the coordinates of bounding boxes
    if x1y1x2y2_box1:  # x1, y1, x2, y2 = box1
        b1_x1, b1_y1, b1_x2, b1_y2 = box1[0], box1[1], box1[2], box1[3]
    else:  # transform from xywh to xyxy
        b1_x1, b1_x2 = box1[0] - box1[2] / 2, box1[0] + box1[2] / 2
        b1_y1, b1_y2 = box1[1] - box1[3] / 2, box1[1] + box1[3] / 2
    ...
    # Intersection area
    inter = (np.min([b1_x2, b2_x2]) - np.max([b1_x1, b2_x1])) *\
            (np.min([b1_y2, b2_y2]) - np.max([b1_y1, b2_y1]))
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, cls_thres=None, classes=None, agnostic=False, multi_label=False,
                        labels=()):
